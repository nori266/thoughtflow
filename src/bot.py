...
def handle_note_creation(note_text, label, prediction, message_id, status):
    note = Thought(
        thought=note_text,
        label=label,
...
def get_text_messages(message):
    ...
    if not category_editing and user.username == ADMIN_USERNAME:
        ...
        status = "open"
        response_message_text: str = format_response_message(message, label, prediction["urgency"], prediction["eta"])
        response_message = bot.send_message(user.id, response_message_text, reply_markup=get_buttons(status), parse_mode='HTML')
        handle_note_creation(message.text, label, prediction, response_message.message_id, status)
